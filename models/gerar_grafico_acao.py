...
def gerar_df_prices_filtrado(country, df_prices, nm_acoes, data_inicial, data_final):
    ...
    # get pct_change (after filter) for each stock and add to new column
    df_prices_filtrado['pct_change_filtro'] = df_prices_filtrado.groupby('nm_acao')['Close'].pct_change().fillna(0)

    acumulados = []
    for nm_acao, group in df_prices_filtrado.groupby("nm_acao"):
        acumulado = list((1 + group["pct_change_filtro"]).cumprod().fillna(1).values)
